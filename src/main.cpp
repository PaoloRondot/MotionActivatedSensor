 ...
 * @version 1.3.4
 * @date 01-09-2022
 *  @feature Ajout de delayFetch : le delai entre deux fetch peut être réglé en minutes
 *    avec NB_SON
 * 
 * @version 1.4.0 
 * @date 02-09-2022
 *  @feature Ajout de delayBefore : le délai entre le moment où le mouvement est détecté
 *    et le déclenchement du son peut se régler depuis le site web
 ...
 *  @feature Augmentation de la mémoire : la mémoire SPIFFS est augmentée à 2.5Mo
 *    aucun son ne peut être installé passé cette limite
 ...
 * @version 1.4.2.2
 *  @date 06-09-2022
 *   @correctif Correction fetch écrasement de tous les sons online avant un fetch
 * 
 * @version 1.4.2.3
 *  @date 06-09-2022
...
// String idModule = "714204fa807437507bc06ac91177ba54";
// const uint8_t fingerprint[20] = { 0xab, 0x0b, 0x34, 0x56, 0x21, 0xce, 0x52, 0x9e, 0xf0, 0x3b, 0xcf, 0x9f, 0xc7, 0xfd, 0x2c, 0x35, 0xd5, 0xeb, 0x4b, 0xef };
...
void setup()
{
  ...
  if (capteurType == CAP_PIR)
    ...
    capteur = new Bouton(D0, scenario);
...
void loop() {
  ...
  if (seconds >= 60) {
    ...
    Serial.print("Nb fetch: ");Serial.println(nbFetch);
    timeLast = timeNow;
    minutes = minutes + 1;
    fetch = true;
    // checkRestart();
  ...
  if (capteur->isTriggered(delaySinceActMin, delaySinceActSec, letsgo)) {
    ...
    if(!letsgo) {
      ...
      Serial.print(F("Titre: "));Serial.println(allSoundsStored[allIndexes[currentIndex++]].title);
...
void checkRestart()
{
  ...
  if ((currentHour == TIME_HOURS_RESTART) && (currentMinute == TIME_MINS_RESTART))
    ESP.restart();
}
...
void fetchAudiosOnline()
{
  ...
  if (https.begin(*client, F("https://connect.midi-agency.com/module/tracks?id_module=") + idModule)) {
  ...
  if (payload.indexOf("\"is_error\":false") != -1){
    int lastIndex = 0;
    int index = 0;
    String delayMinSet_str = payload.substring(payload.indexOf("\"delayMin\":") + 11, payload.indexOf(",\"delaySec\":", lastIndex));
    String delaySecSet_str = payload.substring(payload.indexOf("\"delaySec\":") + 11, payload.indexOf(",\"delayBefSec\":", lastIndex));
    String delayBefSecSet_str = payload.substring(payload.indexOf("\"delayBefSec\":") + 14, payload.indexOf(",\"data\":", lastIndex));
    ...
    while(true) {
      if(payload.indexOf("\"id\":", lastIndex+1) != -1) {
        ...
        String id = payload.substring(payload.indexOf("\"id\":", lastIndex-1) + 5, payload.indexOf(",\"t\":", lastIndex));
        String title = payload.substring(payload.indexOf("\"t\":", lastIndex) + 5, payload.indexOf("\",\"s\":", lastIndex));
        String size = payload.substring(payload.indexOf("\"s\":", lastIndex) + 4, payload.indexOf("\"}", lastIndex));
        int idNum = id.toInt();
  ...
  }else Serial.println(F("is_error = true"));
}
...
void deleteTooMuch()
{
  ...
  for (int stored = 0; stored < NB_SON; ++stored) {
    ...
    for (int online = 0; online < NB_SON; ++online) {
      if (allSoundsOnline[online].title == allSoundsStored[stored].title) {
        newAllSoundStored[index] = allSoundsStored[stored]; // On copie l'ancien dans le nouveau
        newAllSoundStored[index++].id = allSoundsOnline[online].id; // On rajoute l'id
        toRemove = false;
        break;
      }
    }
...
void updateAudios() 
{
  ...
  for (unsigned char online = 0; online < NB_SON; ++online) {
    ...
    for (unsigned char stored = 0; stored < NB_SON; ++stored) {
      // Si dans les fichiers locaux, on a déjà le fichier online correspondant, on copie les info si
      // nécessaire et on passe. Sinon, on lève un flag 'toUpdate'
      if (allSoundsOnline[online].title == allSoundsStored[stored].title) {
        newAllSoundStored[index] = allSoundsStored[stored]; // On copie l'ancien dans le nouveau
        newAllSoundStored[index++].id = allSoundsOnline[online].id; // On rajoute l'id
        toUpdate = false;
    ...
    if (toUpdate) {
      ...
      if(downloadAudio(allSoundsOnline[online]) == 0) {
        if(checkSoundIntegrity(allSoundsOnline[online], "/" + allSoundsOnline[online].title) == 0) {
          newAllSoundStored[index].id = allSoundsOnline[online].id;
          newAllSoundStored[index].size = allSoundsOnline[online].size;
          newAllSoundStored[index].path = "/" + allSoundsOnline[online].title;
          newAllSoundStored[index++].title = allSoundsOnline[online].title;
          Serial.println(F("Audio properly installed"));
        } else {
          Serial.println(F("Audio was deleted because not complete"));
        }
      } else Serial.println(F("Audio could not be downloaded"));
    }
  }
  // Enfin, il faut supprimer les sons qui sont en trop en comparant l'ancienne liste et la nouvelle.
  // Si on trouve un son de l'ancienne liste dans la nouvelle, on passe. Sinon, c'est que ce son 
  // n'existe plus dans la nouvelle liste, et il faut le supprimer.
  for (unsigned char previous = 0; previous < NB_SON; ++previous){
    bool stillThere = false;
    for(unsigned char newList = 0; newList < NB_SON; ++newList) {
      if (allSoundsStored[previous].title == newAllSoundStored[newList].title) {
        stillThere = true;
        break;
      }
    }
...
int checkSoundIntegrity(t_sound toCheck, String path)
{
  ...
  if (sizeFile == toCheck.size) return 0;
  else {
    if (SD.remove(path)) {
...
int downloadAudio(t_sound soundToUpdade)
{
  ...
  Serial.print(F("[HTTPS] begin...\n"));
  if (https.begin(*client, F("https://connect.midi-agency.com/") + path_brute)) {
    ...
    if (httpCode > 0) {
      ...
      //   audioName = audioName.substring(0,audioName.indexOf("\\")) + audioName.substring(audioName.indexOf("\\")+1);
      ...
      if(f) {
        ...
        while (https.connected() && (len > 0 || len == -1)) {
          ...
          f.write(buff, c);
          if (++nbBytes % 100 == 0) Serial.printf("NE PAS DEBRANCHER\n\tnbBytes: %d\n", nbBytes);
          
          if (nbBytes % 10 == 0) {
            digitalWrite(LED_BUILTIN, blink);
            blink = !blink;
          }
          if (!c) { Serial.println(F("read timeout")); }
